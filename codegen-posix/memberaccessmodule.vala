...
public abstract class Vala.CodeGen.PosixMemberAccessModule : Vala.CodeGen.PosixControlFlowModule {
		...
		if (expr.symbol_reference is Method) {
			...
			if (expr.inner is BaseAccess) {
				if (m.base_method != null) {
					CCodeExpression maccess;
					var base_class = (Class) m.base_method.parent_symbol;

					maccess = new CCodeCastExpression(new CCodeIdentifier("%s_type".printf(get_ccode_lower_case_name (current_class.base_class, null))), "%sClass *".printf(get_ccode_name (base_class)));  // current_class new CCodeIdentifier ("%s_type".printf (get_ccode_lower_case_name (base_class, null)));
					set_cvalue (expr, new CCodeMemberAccess.pointer (maccess, get_ccode_vfunc_name (m)));
					return;
