...
public abstract class Vala.CodeGen.PosixBaseModule : Vala.CodeGenerator {
	...
	public override void emit (CodeContext context) {
		...
		if (context.header_filename != null) {
			bool ret = header_file.store (context.header_filename, null, context.version_header, false);
			if (!ret) {
	...
	public void append_strcmp0 () {
		cfile.add_include ("string.h");
		var fun = new CCodeFunction ("strcmp0", "int");
		fun.modifiers = CCodeModifiers.STATIC;
		fun.add_parameter (new CCodeParameter ("str1", "const char*"));
		fun.add_parameter (new CCodeParameter ("str2", "const char*"));
		push_function (fun);
		ccode.open_if (
			new CCodeBinaryExpression (CCodeBinaryOperator.AND,
			new CCodeBinaryExpression (CCodeBinaryOperator.EQUALITY, new CCodeIdentifier ("str1"), new CCodeIdentifier ("NULL")),
			new CCodeBinaryExpression (CCodeBinaryOperator.INEQUALITY, new CCodeIdentifier ("str1"), new CCodeIdentifier ("NULL"))));
		ccode.add_return (new CCodeConstant ("1"));
		ccode.else_if (new CCodeBinaryExpression (CCodeBinaryOperator.AND,
			new CCodeBinaryExpression (CCodeBinaryOperator.EQUALITY, new CCodeIdentifier ("str2"), new CCodeIdentifier ("NULL")),
			new CCodeBinaryExpression (CCodeBinaryOperator.INEQUALITY, new CCodeIdentifier ("str1"), new CCodeIdentifier ("NULL"))));
		ccode.add_return (new CCodeConstant ("-1"));
		ccode.else_if (new CCodeBinaryExpression (CCodeBinaryOperator.AND,
			new CCodeBinaryExpression (CCodeBinaryOperator.EQUALITY, new CCodeIdentifier ("str1"), new CCodeIdentifier ("NULL")),
	...
	public override void visit_property_accessor (PropertyAccessor acc) {
		...
		if (prop.is_abstract || prop.is_virtual) {
			...
			if (prop.parent_symbol is Interface) {
				...
				vcast = new CCodeFunctionCall (new CCodeIdentifier ("TYPE_GET_INTERFACE"));
				vcast.add_argument (new CCodeIdentifier ("self"));
				vcast.add_argument (new CCodeIdentifier (get_ccode_upper_case_name (iface, null)));
			} else {
	...
	public override void visit_block (Block b) {
		...
		if (b.captured) {
			...
			var data_alloc = new CCodeFunctionCall (new CCodeIdentifier ("calloc"));
	...
	private string generate_struct_equal_function (Struct st) {
		...
		foreach (Field f in st.get_fields ()) {
			...
			if (!(f.variable_type is NullType) && f.variable_type.compatible (string_type)) {
				var ccall = new CCodeFunctionCall (new CCodeIdentifier ("strcmp0"));
	...
	public virtual CCodeExpression destroy_value (TargetValue value, bool is_macro_definition = false) {
		...
		if (ccall.call is CCodeIdentifier && !(type is ArrayType) && !is_macro_definition) {
			...
			/* can' be applied since unref0 is a macro
			var cl = type.data_type as Class;
		...
		if (!(expr.left.value_type is NullType)
			...
			if (expr.operator == BinaryOperator.PLUS) {
				...
				if (expr.left.is_constant () && expr.right.is_constant ()) {
					...
					var ccall = new CCodeFunctionCall (new CCodeIdentifier ("strcat"));
	...
	CCodeExpression? create_type_check (CCodeNode ccodenode, DataType type) {
		...
		if (et != null && et.error_code != null) {
			...
			type_id = "%s_type".printf (get_ccode_lower_case_name (type));
			if (type_id == "") {
				return new CCodeInvalidExpression ();
			}
			var ccheck = new CCodeFunctionCall (new CCodeIdentifier ("object_is_subtype_of"));
	...
	public TargetValue transform_value (TargetValue value, DataType? target_type, CodeNode node) {
		...
		bool gvalue_boxing = false;
		bool gvariant_boxing = false;
		...
		if (!gvalue_boxing && !gvariant_boxing && target_type.value_owned && (!type.value_owned || boxing || unboxing) && requires_copy (target_type) && !(type is NullType)) {
			...
			if (target_type.data_type is Interface && copy == null) {
				//HACK: interfaces don't require copies?!?!
	...
	public CCodeFunctionCall trace_method_enter (string method_name) {
		var print_call = new CCodeFunctionCall (new CCodeIdentifier ("fprintf"));
		print_call.add_argument (new CCodeIdentifier ("stderr"));
		print_call.add_argument (new CCodeConstant ("\"TRACE - METHOD - ENTER: %s\\n\"".printf (method_name)));

		return print_call;
	}

	public CCodeFunctionCall trace_method_leave (string method_name) {
		var print_call = new CCodeFunctionCall (new CCodeIdentifier ("fprintf"));
		print_call.add_argument (new CCodeIdentifier ("stderr"));
		print_call.add_argument (new CCodeConstant ("\"TRACE - METHOD - LEAVE: %s\\n\"".printf (method_name)));
