...
public class Vala.CodeGen.PosixMethodCallModule : Vala.CodeGen.PosixAssignmentModule {
	public override void visit_method_call (MethodCall expr) {
		...
		if (itype is MethodType) {
			...
			} else if (ma.inner != null && ma.inner.value_type != null && ma.inner.value_type.data_type == string_type.data_type && ma.member_name == "printf") {
		...
		if (m is CreationMethod && m.parent_symbol is Class) {
			...
			var self = get_this_cexpression ();
			var cl = (Class) m.parent_symbol;
			if (cl.base_class != null) {
				
				self = new CCodeCastExpression(self, "%s *".printf(get_ccode_name(cl)));
			}
			ccall.add_argument (self);
