 ...
 */
public abstract class Vala.CodeGen.PosixMethodModule : Vala.CodeGen.PosixStructModule {
	...
	public override void generate_method_declaration (Method m, CCodeFile decl_space) {
		...
		if (m is CreationMethod && cl != null) {
			...
			real_name = get_ccode_real_name (m);
			...
			function.return_type = get_creturn_type (m, get_ccode_name (new VoidType ()));
	...
	public override void visit_method (Method m) {
		...
		if (profile) {
			...
			var counter_decl = new CCodeDeclaration ("int");
			...
			var level_decl = new CCodeDeclaration ("int");
			...
			*/
			...
			print_call.add_argument (new CCodeConstant ("\"%s: (%%d calls)\\n\"".printf (m.get_full_name ())));
		...
		if (m is CreationMethod) {
			creturn_type = new VoidType ();
			function.return_type = get_creturn_type (m, get_ccode_name (creturn_type));
		}
		...
		if (trace_method_call) {
			ccode.add_expression (trace_method_enter (m.get_full_name ()));
		}
		...
		if (trace_method_call) {
			ccode.add_expression (trace_method_leave (m.get_full_name ()));
		}
		if (profile) {
			string prefix = "_vala_prof_%s".printf (real_name);
		...
		if (m.is_abstract && current_type_symbol is Class) {
			...
			cfile.add_include ("stdio.h");
			var error_string = "\"Type `%%p' does not implement abstract method `%s'\\n\"".printf (get_ccode_name (m));

			var cerrorcall = new CCodeFunctionCall (new CCodeIdentifier ("fprintf"));
			cerrorcall.add_argument (new CCodeIdentifier ("stderr"));
			cerrorcall.add_argument (new CCodeConstant (error_string));
			cerrorcall.add_argument (new CCodeIdentifier ("self"));
	...
	public override void generate_cparameters (Method m, CCodeFile decl_space, Map<int,CCodeParameter> cparam_map, CCodeFunction func, CCodeFunctionDeclarator? vdeclarator = null, Map<int,CCodeExpression>? carg_map = null, CCodeFunctionCall? vcall = null, int direction = 3) {
		if (m.closure) {
			...
			if (!cl.is_compact && vcall == null && (direction & 1) == 1) {
				cparam_map.set (get_param_pos (get_ccode_instance_pos (m)), new CCodeParameter ("self", "%s *".printf(get_ccode_name(cl))));
			}
	...
	public void generate_vfunc (Method m, DataType return_type, Map<int,CCodeParameter> cparam_map, Map<int,CCodeExpression> carg_map, string suffix = "", int direction = 3) {
		...
		if (m.parent_symbol is Interface) {
			...
			vcast = new CCodeFunctionCall (new CCodeIdentifier ("%s_GET_TYPE".printf (get_ccode_upper_case_name (cl))));
		}
	...
	private void create_aux_constructor (CreationMethod m, string func_name, bool self_as_first_parameter) {
		...
		var ccall = new CCodeFunctionCall (new CCodeIdentifier ("%s_type_init".printf(get_ccode_lower_case_name (cl))));
		ccode.add_expression(ccall);
		
		ccall = new CCodeFunctionCall (new CCodeIdentifier ("calloc"));
		ccall.add_argument (new CCodeConstant("1"));
		ccall.add_argument (new CCodeIdentifier ("sizeof (%s)".printf (get_ccode_name (cl))));
		ccode.add_assignment (get_this_cexpression (), new CCodeCastExpression (ccall, get_ccode_name (cl) + "*"));
		
		
		//if (cl.base_class != null) {
		var left = new CCodeMemberAccess.pointer(new CCodeCastExpression(new CCodeIdentifier ("self"), "Object *"), "type");
		var right = new CCodeCastExpression(new CCodeIdentifier ("%s_type".printf(get_ccode_lower_case_name (cl))), "Type *");
		ccode.add_assignment (left, right);
		//} else {
		//	ccode.add_assignment (new CCodeMemberAccess.pointer (new CCodeIdentifier ("self"), "class"), new CCodeIdentifier ("%s_type".printf(get_ccode_lower_case_name (cl))));
		//}
		
		/*
		ccall = new CCodeFunctionCall(new CCodeIdentifier ("%s_instance_init".printf(get_ccode_lower_case_name (cl))));
		ccall.add_argument (new CCodeIdentifier("self"));
		ccode.add_expression(ccall);
		*/
		
		if (self_as_first_parameter) {
			cparam_map.set (get_param_pos (get_ccode_instance_pos (m)), new CCodeParameter ("self", get_ccode_name (cl)));
			vcall.add_argument (get_variable_cexpression ("self"));
		} else {
