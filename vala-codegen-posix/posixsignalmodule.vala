...
public class  Vala.CodeGen.PosixSignalModule : Vala.CodeGen.PosixClassModule {
	...
	void generate_marshaller (List<Parameter> params, DataType return_type) {
		...
		if (user_marshal_set.contains (signature)) {
			return;
		}
		...
		signal_marshaller.add_parameter (new CCodeParameter ("instance", "void *"));
		signal_marshaller.add_parameter (new CCodeParameter ("callback", "void *"));
		signal_marshaller.add_parameter (new CCodeParameter ("user_data", "void *"));
		signal_marshaller.add_parameter (new CCodeParameter ("return_value", "void *"));
		signal_marshaller.add_parameter (new CCodeParameter ("args", "va_list"));

		push_function (signal_marshaller);
		
		var callback_decl = new CCodeFunctionDeclarator (get_marshaller_function (params, return_type, "MarshalFunc"));
		callback_decl.add_parameter (new CCodeParameter ("sender", "void *"));
		n_params = 1;
		foreach (Parameter p in params) {
		...
		callback_decl.add_parameter (new CCodeParameter ("user_data", "void *"));
		ccode.add_statement (new CCodeTypeDefinition (get_value_type_name_from_type_reference (return_type), callback_decl));

		ccode.add_declaration (get_marshaller_function (params, return_type, "MarshalFunc"), new CCodeVariableDeclarator ("event_callback", new CCodeIdentifier ("callback")), CCodeModifiers.REGISTER);
		...
		*/
		...
		fc = new CCodeFunctionCall (new CCodeIdentifier ("event_callback"));
		fc.add_argument (new CCodeIdentifier ("instance"));
		foreach (Parameter p in params) {
			string get_value_function;
			bool is_array = p.variable_type.is_array ();
			if (p.direction != ParameterDirection.IN) {
				get_value_function = "void*";
			} else if (is_array) {
			...
			var inner_fc = new CCodeFunctionCall (new CCodeIdentifier ("va_arg"));
			inner_fc.add_argument (new CCodeIdentifier ("args"));
			inner_fc.add_argument (new CCodeIdentifier (get_value_function));
			...
			if (is_array) {
				inner_fc = new CCodeFunctionCall (new CCodeIdentifier ("va_arg"));
				inner_fc.add_argument (new CCodeIdentifier ("args"));
				inner_fc.add_argument (new CCodeIdentifier ("int"));
				fc.add_argument (inner_fc);
			}
		}
		fc.add_argument (new CCodeIdentifier ("user_data"));
	...
	public override void visit_element_access (ElementAccess expr) {
		if (expr.container is MemberAccess && expr.container.symbol_reference is Signal) {
			if (expr.parent_node is MethodCall) {
				...
				var ccall = new CCodeFunctionCall (new CCodeIdentifier ("object_signal_emit"));
	...
	public override void visit_member_access (MemberAccess expr) {
		if (expr.symbol_reference is Signal) {
			...
			if (get_signal_has_emitter (sig)) {
				...
				ccall.add_argument (new CCodeMemberAccess.pointer (pub_inst, sig.name)); //get_signal_canonical_constant (sig));
				ccall.add_argument (new CCodeIdentifier (get_marshaller_function (sig.get_parameters (), sig.return_type, null)));
				if (sig.return_type.data_type == null) {
					ccall.add_argument (new CCodeIdentifier ("NULL"));
				} else {
					var temp_var = get_temp_variable (sig.return_type);

					emit_temp_var (temp_var);
					ccall.add_argument (new CCodeIdentifier (temp_var.name));
				}
				set_cvalue (expr, ccall);
			}
		} else {
	...
	CCodeExpression? connect_signal (Signal sig, Expression signal_access, Expression handler, bool disconnect, bool after, CodeNode expr) {
		...
		if (!disconnect) {
			...
			if (sig is DynamicSignal) {
				...
				if (m.closure) {
					connect_func = "object_signal_connect";
				} else if (!after) {
					connect_func = "object_signal_connect";
				} else {
					connect_func = "object_signal_connect";
			...
			if (sig is DynamicSignal) {
				...
				connect_func = "object_signal_disconnect_callback";
			}
		}
		...
		if (sig is DynamicSignal) {
			...
			// second argument: signal name
			//ccall.add_argument (signal_name_cexpr);
			CCodeExpression self;
			if (ma.inner != null) {
				self = (CCodeExpression) get_ccodenode (ma.inner);
			} else {
				self = get_result_cexpression ("self");
			}
			ccall.add_argument (new CCodeUnaryExpression(CCodeUnaryOperator.ADDRESS_OF, new CCodeParenthesizedExpression (new CCodeMemberAccess.pointer (self, sig.name))));
		} else {
			// object_signal_disconnect_callback

			// second argument: signal handler list
			CCodeExpression self;
			if (ma.inner != null) {
				self = (CCodeExpression) get_ccodenode (ma.inner);
			} else {
		...
		ccall.add_argument (get_cvalue (handler));
		...
		if (m.closure) {
			...
			ccall.add_argument (new CCodeCastExpression (handler_destroy_notify, "NotifyCallback"));
			...
			*/
			ccall.add_argument (new CCodeConstant ("NULL")); // notify
		} else {
			...
			if (!disconnect) {
				ccall.add_argument (new CCodeConstant ("NULL")); // user_data
				ccall.add_argument (new CCodeConstant ("NULL")); // notify
			}
		}
